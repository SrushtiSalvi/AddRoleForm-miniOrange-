...
import userCapabilities from "../../usercapabilities.json";
import { useDispatch, useSelector } from "react-redux";
import { setUserCapabilities } from "../../store/slices/userSlice";
...
const UserCapabilities = () => {
  ...
  const listData = useSelector((state) => state.user.userCapabilities);
  ...
  useEffect(() => {
    ...
    // const isAdmin = true;
    // if (user.inherit == "Admin") {
    //   const updatedCapabilities = adminCapabilities.map((item) => {
    //     item.capabilities.map((capability) => {
    //       capability.checked = true;
    //       capability.subcategories.map((Category) => {
    //         capability.checked = true;
    //       });
    //     });
    //   });
    //   dispatch(setUserCapabilities(updatedCapabilities));
    // }
    if (listData.length === 0) {
      console.log(
        "No data in redux, setting data on redux for userCapabilities"
      );
      if (user.inheritFrom === "User") {
        const result = updateCheckedStatus(userCapabilities);
        // setResultData(result);
        dispatch(setUserCapabilities(result));
  ...
  const handleChildCheckBox = (parentId, capabilityId, subcategoryId) => {
    ...
    dispatch(setUserCapabilities(updatedData));
  };
